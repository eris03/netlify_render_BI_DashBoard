document.addEventListener('DOMContentLoaded', () => {
    ...
    const BACKEND_URL = 'https://netlify-render-bi-dashboard-1-2vcl.onrender.com';
    ...
    const showLoadingState = () => {
        ...
        document.getElementById('key-metrics').innerHTML = `
            <div class="flex justify-between"><span class="text-gray-300">Volume:</span> <span class="skeleton h-5 w-20 bg-gray-700 rounded-md"></span></div>
            <div class="flex justify-between"><span class="text-gray-300">Day High:</span> <span class="skeleton h-5 w-20 bg-gray-700 rounded-md"></span></div>
            <div class="flex justify-between"><span class="text-gray-300">Day Low:</span> <span class="skeleton h-5 w-20 bg-gray-700 rounded-md"></span></div>
            <div class="flex justify-between"><span class="text-gray-300">Market Cap:</span> <span class="skeleton h-5 w-20 bg-gray-700 rounded-md"></span></div>`;
    };
    ...
    const updateUI = (profile, quote) => {
        ...
        logo.src = profile.image || `https://placehold.co/200x200/1f2937/ffffff?text=${profile.symbol}`;
        logo.onerror = () => {
            logo.src = `https://placehold.co/200x200/1f2937/ffffff?text=${profile.symbol}`;
        };
        logo.classList.remove('hidden');

        const logoLoader = document.getElementById('company-logo-loader');
    ...
    const renderChart = async (timeframe) => {
        document.getElementById('chart-loader').style.display = 'flex';
        document.getElementById('chart-loader').innerHTML = ''; // Reset loader message
        document.getElementById('price-chart').style.opacity = '0';
        ...
        if (timeframe === 'intraday') {
            ...
            if (!state.dailyData) {
                try {
                    ...
                    console.error("Daily chart fetch error:", err);
